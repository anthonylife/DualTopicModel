...
double LDA::EvalTrainPerp() {
    unsigned int total_word=0;
    double perplexity = 0.0, word_log_prob = 0.0;
    double beta_sum = m_beta * m_corpus.m_vocab.size();
    double alpha_sum = m_alpha * m_topic_num;
    unsigned int docs_num = m_corpus.m_docs.size();
    ...
    for (unsigned int m = 0; m < docs_num ; ++m ) {
        ...
        for (unsigned int n = 0; n < doc_len ; ++n ) {
            ...
            for (unsigned int k = 0; k < m_topic_num; k++) {
	            ...
	            word_log_prob += (m_Nw[w][k] + m_beta) / (m_NAll[k] + beta_sum) *
		        (m_Nd[m][k] + as_prior) / (doc_len -1 + alpha_sum);
            }
            if (isnan(word_log_prob) || word_log_prob == 0)  {
                printf("Nan or 0 for word_log_prob: %lf\n", word_log_prob);
                exit(1);
            }
            perplexity += log(word_log_prob);
        }
    }
    return exp(-perplexity/total_word);
}
    ...
    for (unsigned int m = 0; m < docs_num ; ++m ) {
        ...
        for (unsigned int n = 0; n < doc_len ; ++n ) {
            ...
            for (unsigned int k = 0; k < m_topic_num; k++) {
	            ...
	            word_log_prob += (m_inf_Nw[w][k] + m_Nw[w][k] + m_beta) 
                    / (m_inf_NAll[k] + m_NAll[k] + beta_sum) *
		            (m_inf_Nd[m][k] + as_prior) / (doc_len -1 + alpha_sum);
            }
            if (isnan(word_log_prob) || word_log_prob == 0)  {
                printf("Nan or 0 for word_log_prob: %lf\n", word_log_prob);
...
bool LDA::Estimate()
{
    ...
    for (unsigned int iter = 0; iter < m_max_iter; ++iter )
    {
        ...
        double perp = EvalTrainPerp();
        std::cout << "Perplexity of training data: " << perp << std::endl;
    }
...
bool LDA::Inference(const std::string& pre_model)
{
    ...
    for (unsigned int iter = 0; iter < m_max_iter; ++iter )
    {
        ...
        double perp = EvalTestPerp();
        std::cout << "Perplexity of test data: " << perp << std::endl;
    }
