...
int model::save_test_theta() {
    FILE * fout = fopen("test-final.theta", "w");
    if (!fout) {
	printf("Cannot open file test-final.theta to save!\n");
	return 1;
    }
...
void model::estimate() {
    ...
    compute_theta();
    compute_phi();
    double perp = evalTrainPerplexity();
    printf("Perplexity of training data: %lf\n", perp);
    }
...
void model::inference() {
    ...
    compute_newtheta();
    compute_newphi();
    double test_perplexity = evalTestPerplexity();
    printf("Perplexity on testing data: %.4f!\n", test_perplexity);
    }
...
double model::evalTrainPerplexity() {
    ...
    for (int m = 0; m < M; m++) {
        //word_cnt += pnewdata->docs[m]->length;
	    for (int n = 0; n < ptrndata->docs[m]->length; n++) {
            w = ptrndata->docs[m]->words[n];
            word_log_prob = 0.0;
            word_cnt += 1;
            for (int k=0; k < K; k++)
                word_log_prob += theta[m][k]*phi[k][w];
            if (isnan(word_log_prob) || word_log_prob == 0) {
                printf("word_log_prob:%lf", word_log_prob);
                exit(1);
            }
            perplexity += log(word_log_prob);
        }
    }

    return exp(-perplexity/word_cnt);
}

double model::evalTestPerplexity() {
    double perplexity = 0.0;
	double word_log_prob = 0.0;
    int w;
    int word_cnt = 0;
    ...
    for (int m = 0; m < newM; m++) {
        //word_cnt += pnewdata->docs[m]->length;
	    for (int n = 0; n < pnewdata->_docs[m]->length; n++) {
            w = pnewdata->_docs[m]->words[n];
            word_log_prob = 0.0;
            if (w >= newV) {
                //for (int k=0; k < K; k++)
                //    word_log_prob += newtheta[m][k]*1.0/newV;
                continue;
            }
            word_cnt += 1;
            for (int k=0; k < K; k++)
                word_log_prob += newtheta[m][k]*newphi[k][w];
            if (word_log_prob == 0) {
                printf("word_log_prob = 0\n");
            	printf("newtheta[%d]:", m);
                for (int k=0; k < K; k++)
		            printf(" %f", newtheta[m][k]);
                printf("\n");
                printf("newphi[k][%d]:", w);
                for (int k=0; k<K; k++)
                    printf(" %f", newphi[k][w]);
